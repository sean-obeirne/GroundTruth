...
static void cmd_read(char *args[], uint8_t argc)
{
    if (argc < 1)
    {
        cmd_respond("ERR READ requires pin");
        return;
    }
    pin_id_t id = pin_parse(args[0]);
    if (id == PIN_INVALID)
    {
        cmd_respond("ERR unknown pin");
        return;
    }
    if (!pin_has_cap(id, PIN_CAP_INPUT))
    {
        cmd_respond("ERR pin not input");
        return;
    }
...
static void cmd_voltage(char *args[], uint8_t argc)
{
    if (argc < 1)
    {
        cmd_respond("ERR VOLTAGE requires pin");
        return;
    }
    pin_id_t id = pin_parse(args[0]);
    if (id == PIN_INVALID)
    ...
    if (!pin_has_cap(id, PIN_CAP_ANALOG))
    {
        cmd_respond("ERR pin not analog");
        return;
    }
...
static void cmd_set(char *args[], uint8_t argc)
{
    if (argc < 2)
    {
        cmd_respond("ERR SET requires pin and HIGH/LOW");
        return;
    }
    pin_id_t id = pin_parse(args[0]);
    if (id == PIN_INVALID)
    ...
    if (!pin_has_cap(id, PIN_CAP_OUTPUT))
    {
        cmd_respond("ERR pin not output");
        return;
    }

    uint8_t val;
    if (strcasecmp(args[1], "HIGH") == 0)
        val = 1;
    else if (strcasecmp(args[1], "LOW") == 0)
        val = 0;
    else
    {
        cmd_respond("ERR expected HIGH or LOW");
        return;
    }
...
static void cmd_watch(char *args[], uint8_t argc)
{
    if (argc < 2)
    {
        cmd_respond("ERR WATCH requires pin and duration_ms");
        return;
    }
    pin_id_t id = pin_parse(args[0]);
    if (id == PIN_INVALID)
    ...
    if (ms == 0 || ms > 30000)
    {
        cmd_respond("ERR duration 1-30000 ms");
        return;
    }
...
static void cmd_edges(char *args[], uint8_t argc)
{
    if (argc < 2)
    {
        cmd_respond("ERR EDGES requires pin and duration_ms");
        return;
    }
    pin_id_t id = pin_parse(args[0]);
    if (id == PIN_INVALID)
...
static void cmd_pulse(char *args[], uint8_t argc)
{
    if (argc < 3)
    {
        cmd_respond("ERR PULSE requires pin count delay_us");
        return;
    }
    pin_id_t id = pin_parse(args[0]);
    if (id == PIN_INVALID)
    ...
    {
        cmd_respond("ERR count 1-10000");
        return;
    }
    if (delay == 0 || delay > 50000)
    {
        cmd_respond("ERR delay_us 1-50000");
        return;
    }
...
static void cmd_button_test(char *args[], uint8_t argc)
{
    if (argc < 2)
    {
        cmd_respond("ERR BUTTON_TEST requires pin and duration_ms");
        return;
    }
    pin_id_t id = pin_parse(args[0]);
    if (id == PIN_INVALID)
    ...
    if (ms == 0 || ms > 60000)
    {
        cmd_respond("ERR duration 1-60000 ms");
        return;
    }
...
static void cmd_encoder_test(char *args[], uint8_t argc)
{
    if (argc < 3)
    {
        cmd_respond("ERR ENCODER_TEST requires pinA pinB duration_ms");
        return;
    }
    ...
    {
        cmd_respond("ERR pins must be input");
        return;
    }
...
static void dispatch(char *line)
{
    ...
    if (strcmp(tokens[0], "PING") == 0)
        ...
        cmd_read(args, argc);
    else if (strcmp(tokens[0], "VOLTAGE") == 0)
        cmd_voltage(args, argc);
    else if (strcmp(tokens[0], "SET") == 0)
        cmd_set(args, argc);
    else if (strcmp(tokens[0], "WATCH") == 0)
        cmd_watch(args, argc);
    else if (strcmp(tokens[0], "EDGES") == 0)
        cmd_edges(args, argc);
    else if (strcmp(tokens[0], "RAILS") == 0)
        cmd_rails();
    else if (strcmp(tokens[0], "PULSE") == 0)
        cmd_pulse(args, argc);
    else if (strcmp(tokens[0], "BUTTON_TEST") == 0)
        cmd_button_test(args, argc);
    else if (strcmp(tokens[0], "ENCODER_TEST") == 0)
        cmd_encoder_test(args, argc);
    else
    {
